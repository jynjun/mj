...
import type { DeathCause, RoleId } from '@mj/game-engine';
import { useGame } from '../game';
import { useSound } from '../sound';
import { NightPhase } from './NightPhase';
import { DawnPhase, DayPhase, VotePhase, HunterPhase, VictoryScreen } from './DayPhase';

export function GameScreen() {
  const { state, dispatch } = useGame();
  const { playSound, playingEventId, enabled, setShowModal } = useSound();
  const [showHistory, setShowHistory] = useState(false);

  const { gamePhase, nightCount, timerSeconds, timerRunning, layoutMode } = state;
  const isNight = gamePhase === 'night' || gamePhase === 'dawn';
  ...
  function PhaseContent() {
    if (gamePhase === 'night') return <NightPhase />;
    if (gamePhase === 'dawn') return <DawnPhase />;
    if (gamePhase === 'day') return <DayPhase />;
    if (gamePhase === 'vote') return <VotePhase />;
    if (gamePhase === 'hunter') return <HunterPhase />;
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
      {gamePhase === 'night' && (
        <div style={{ minHeight: '52px', padding: '6px 12px', display: 'flex', alignItems: 'center', flexWrap: 'wrap', rowGap: '6px', borderBottom: '1px solid rgba(180,30,60,0.16)', background: 'rgba(4,2,8,0.96)', flexShrink: 0, zIndex: 10 }}>
          <button onClick={() => dispatch({ type: 'BACK_FROM_NIGHT' })} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', padding: '4px 8px', lineHeight: 1, flexShrink: 0 }}>←</button>
          <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px' }}>
            ...
            <button onClick={() => playSound('night_start')}
              style={{ padding: '5px 12px', background: playingEventId === 'night_start' ? 'rgba(196,30,58,0.18)' : 'transparent', border: '1px solid ' + (playingEventId === 'night_start' ? 'rgba(196,30,58,0.55)' : 'rgba(180,30,60,0.28)'), borderRadius: '7px', cursor: 'pointer', fontFamily: "'Cinzel',serif", fontSize: '10px', color: playingEventId === 'night_start' ? 'var(--red-light)' : 'var(--text-muted)', letterSpacing: '0.12em', flexShrink: 0 }}>
              {playingEventId === 'night_start' ? '■ Ambiance' : 'Ambiance'}
            </button>
          </div>
          <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
            <button onClick={() => setShowHistory(!showHistory)} style={{ background: 'transparent', border: '1px solid rgba(180,30,60,0.22)', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', padding: '4px 8px', color: 'var(--text-muted)', fontFamily: "'Cinzel',serif" }}>📜</button>
            <button onClick={() => setShowModal(true)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '16px', padding: '4px', color: enabled ? 'var(--text-muted)' : 'var(--text-dim)' }}>{enabled ? '🔊' : '🔇'}</button>
          </div>
        </div>
      )}
      ...
      {gamePhase !== 'night' && (
        <div style={{ minHeight: '52px', padding: '6px 14px', display: 'flex', alignItems: 'center', gap: '10px', rowGap: '8px', flexWrap: 'wrap', borderBottom: '1px solid rgba(180,30,60,0.16)', background: 'rgba(4,2,8,0.96)', flexShrink: 0, zIndex: 10 }}>
          ...
          <div style={{ padding: '3px 8px', background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.28)', borderRadius: '12px', fontFamily: "'Cinzel',serif", fontSize: '10px', color: '#34d399', letterSpacing: '0.1em', flex: '0 0 auto' }}>{alive} vivants</div>
          <div style={{ flex: 1 }} />
          {(gamePhase === 'day' || gamePhase === 'vote') && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '17px', fontWeight: 600, color: timerSeconds > 0 && timerSeconds <= 10 ? '#ef4444' : timerSeconds === 0 ? 'var(--text-dim)' : 'var(--text)', minWidth: '44px', textAlign: 'right' }}>
                {timerSeconds > 0 ? fmt(timerSeconds) : '-'}
              </span>
              <div style={{ display: 'flex', gap: '3px' }}>
                {[120, 300, 600].map((s) => (
                  <button key={s} onClick={() => dispatch({ type: 'TIMER_SET', seconds: s })}
                ...
                <button onClick={() => { if (timerSeconds === 0) dispatch({ type: 'TIMER_SET', seconds: 300 }); dispatch({ type: 'TIMER_TOGGLE' }); }}
          ...
          <button onClick={() => setShowHistory(!showHistory)} title="Historique" style={{ background: 'transparent', border: '1px solid rgba(180,30,60,0.22)', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', padding: '4px 8px', color: 'var(--text-muted)', fontFamily: "'Cinzel',serif", flexShrink: 0 }}>📜</button>
          <button onClick={() => setShowModal(true)} title="Sons" style={{ background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '16px', padding: '4px', color: enabled ? 'var(--text-muted)' : 'var(--text-dim)', flexShrink: 0 }}>{enabled ? '🔊' : '🔇'}</button>
          <button onClick={() => dispatch({ type: 'SET_LAYOUT', mode: layoutMode === 'focused' ? 'dashboard' : 'focused' })} title={layoutMode === 'focused' ? 'Vue tableau de bord' : 'Vue focalisee'}
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
          <div style={{ width: '190px', flexShrink: 0, borderRight: '1px solid rgba(180,30,60,0.12)', overflowY: 'auto', padding: '8px', display: 'flex', flexDirection: 'column', gap: '5px', background: 'rgba(4,2,8,0.5)' }}>
            ...
            {state.players.map((player) => {
              ...
              const isVic = state.nightVictimId === player.id;
              return (
                <div key={player.id} style={{ background: isVic ? 'rgba(180,20,20,0.22)' : player.alive ? 'rgba(10,4,18,0.85)' : 'rgba(4,2,8,0.6)', border: '1px solid ' + (isVic ? 'rgba(220,50,50,0.5)' : player.alive ? 'rgba(180,30,60,0.16)' : 'rgba(50,35,45,0.14)'), borderRadius: '8px', padding: '7px 9px', display: 'flex', alignItems: 'center', gap: '7px', opacity: player.alive ? 1 : 0.38 }}>
...
function HistoryPanel() {
  const { state } = useGame();
  const nightsSet: Record<number, boolean> = {};
  state.nightActions.forEach((a) => { nightsSet[a.night] = true; });
  state.deaths.forEach((d) => { nightsSet[d.night] = true; });
  const nights = Object.keys(nightsSet).map(Number).sort((a, b) => a - b);
  const rn = (rid: RoleId) => { const rd = ROLES_DATA[rid]; return rd ? rd.name : rid; };
  const reveal = (rid: RoleId) => rid === 'VOYANTE' || rid === 'FOSSOYEUR' || rid === 'VOLEUR';
  const targets = (a: (typeof state.nightActions)[number]) => {
    if (a.targetIds.length > 0) {
      const s = a.targetIds.map((id) => { const p = state.players.find((pl) => pl.id === id); return 'J' + id + (reveal(a.roleId) && p ? ' (' + rn(p.role) + ')' : ''); }).join(', ');
      return s + (a.note ? ' · ' + a.note : '');
    }
    return a.note || '-';
  };
  const causeFr = (c: DeathCause) => c === 'wolves' ? 'Loups' : c === 'witch' ? 'Sorciere' : c === 'lovers' ? 'Chagrin' : c === 'lynch' ? 'Lynchage' : c === 'hunter' ? 'Chasseur' : c === 'sister' ? 'Vengeance' : '?';
  ...
  return (
    <>
      ...
      {nights.length === 0 ? (
        <div style={{ fontSize: '13px', color: 'var(--text-dim)', fontFamily: "'Crimson Text',serif", fontStyle: 'italic' }}>Rien a afficher pour l'instant.</div>
      ) : nights.map((nt) => {
        const acts = state.nightActions.filter((a) => a.night === nt && (a.targetIds.length > 0 || a.note));
        const dts = state.deaths.filter((d) => d.night === nt);
        return (
          <div key={nt} style={{ marginBottom: '14px' }}>
            ...
            {acts.length === 0 && dts.length === 0 && (<div style={{ fontSize: '12px', color: 'var(--text-dim)', fontFamily: "'Crimson Text',serif", fontStyle: 'italic' }}>Aucune action.</div>)}
            {acts.map((a) => {
              const rd = ROLES_DATA[a.roleId];
              return (
